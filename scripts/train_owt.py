"""Train a 12M-param GPT on an OpenWebText slice (fp32, GPU via CuPy)."""
...
# IMPORTANT: set DTYPE to fp32 BEFORE any Tensor is constructed.
# At fp64, the (B, T, V) logits tensor doesn't fit in 6 GB; fp32 halves it.
...
def main():
    ...
    print(f"backend: {backend.name}  dtype: {backend.DTYPE}")
